...
ソフトウェア開発の世界では、特に受託開発の案件などで、「頼まれた機能さえ動けば十分」という認識でもあったのでしょうか？　テストやリファクタリングなどの品質管理や保守性向上のための活動に十分な時間と労力を割くことを許されていないのだろうと思われるようなプロジェクトが散見されてきました。しかし、現代のモバイルアプリケーション開発において、「アプリを作って終わり」というアプローチは通用しません。アプリのリリース後も、継続的な機能改善や性能向上、バグ修正などが求められる長い旅が待っているのです。

本来、そのような長期的な視点に立った開発体制が必要不可欠ですが、これまでは軽視されがちな傾向にあったように思います。開発者にとっては、「こういう開発のやり方ができれば理想的だ」と思えるような手法や原則を知ることが重要です。一方で、開発の詳細について深い知識を持たない発注者や関係者にとっても、「この本に書かれているようなやり方がベストプラクティスらしいので、これに従った開発をお願いしたい」と指定できるような、わかりやすい指針が必要とされています。

本書は、このような課題意識のもと、高品質なモバイルアプリケーションを継続的に開発・運用していくための原則とプラクティスを提供することを目的としています。特に、クリーンアーキテクチャやテスト駆動開発（TDD）、リファクタリングなどの手法に焦点を当て、それらをAndroidアプリケーション開発に適用する方法を具体的に解説します。

また、本書は、プログラミングの基礎を習得したばかりの人たちが、「実際のプロダクト開発ってどんな手順で進んでいくのだろう？」という疑問に答えることも目指しています。LinkedPalという仮想のアプリケーション開発プロジェクトを通して、要件定義から設計、実装、テスト、リリースに至るまでの一連の流れを追体験できるようになっています。

本書を通じて、読者の皆さんが、品質の高いソフトウェアを継続的に開発・運用できる現場を実現するための知識とスキルを身につけ、自信を持ってプロジェクトに臨めるようになることを願っています。アプリケーションの開発を単なる「作る」行為ではなく、ユーザーに価値を届け続ける「育てる」営みとしてとらえ、日々の開発活動に取り組んでいただければ幸いです。

それでは、LinkedPalプロジェクトを通して、クリーンアーキテクチャとテスト駆動開発の世界を探求していきましょう。
