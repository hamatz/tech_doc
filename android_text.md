# Androidアプリ開発：クリーンアーキテクチャとMVVMによるコード設計入門
...
やりたいことは「Googleなどが公式に示しているAndroidアプリ開発のベストプラクティスに沿った形で進めることで、多くの開発者にとって参画時に戸惑いやストレスが少ない形でコラボレーションできるような状況を維持し続けたい」という比較的シンプルな話なのですが、口で言うほど簡単な話ではありません。

筆者は、Androidアプリケーション開発のためのアーキテクチャドキュメントを作成した際に、この問題に直面しました。アーキテクチャドキュメントは、アプリケーションの設計や構造を説明するために重要ですが、それをチーム内で共有するだけでは不十分であると感じました。アーキテクチャドキュメントの内容を正しく理解し、実践するためには、具体的な実装例やベストプラクティスを示す副読本のようなテキストが必要だと考えました。それがAndroidアプリ開発を学んだ後で多人数での開発に飛び込む前に、個人開発との間にあるギャップを可能な限り埋めたり、チームでの働き方が想像できるものであると尚良い、という意図もありました。

そこで、本書の執筆を始めました。本書は、Androidアプリケーション開発におけるクリーンアーキテクチャとMVVMの実践に焦点を当てた技術書です。実際のコード例を通してアーキテクチャパターンの適用方法を説明します。
...
ゼロから書籍を執筆し、公開することで、Androidアプリケーション開発におけるクリーンアーキテクチャとMVVMの実践に関する知見を広く共有し、アプリケーションの品質と保守性の向上に貢献できればと考えています。
