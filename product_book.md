...
LinkedPalの開発では、アジャイル開発手法を採用します。アジャイル開発とは、短いイテレーション（反復）を繰り返しながら、フィードバックを得て開発を進めていく手法です。従来の開発手法（ウォーターフォール型）が、各フェーズを順番に進めていくのに対し、アジャイル開発では、要件定義、設計、開発、テストなどの工程を小さなイテレーションに分割し、それらを繰り返し行います。これにより、変化に柔軟に対応し、お客様のフィードバックを早い段階で取り入れることができます。

LinkedPalの開発では、具体的に以下のようなアジャイルプラクティスを取り入れます：
...
以上で、「プロセスを眺めて学ぶプロダクト開発入門」を終わります。読者の皆さんが、本書で得た知識を活かし、より良いプロダクトを開発されることを祈っています。
