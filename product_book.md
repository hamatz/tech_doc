...
多くの企業では、既存顧客のさらなる販売拡大を目的としたプロダクト開発が行われます。その場合、プロダクトの目的を定義するには、以下のような視点が重要です。

1. 既存顧客の中で、特にターゲットとしたい層を明確にする
   - 売上への貢献度が高い層や、ニーズが満たされていない層など、ターゲットとなる顧客層を特定する

2. ターゲット層の課題やニーズを深掘りする
   - 既存プロダクトやサービスでは満たされていない、ターゲット層特有の課題やニーズを見つける
   - 潜在的なニーズを掘り起こすことも重要

3. 課題解決のためのプロダクトのあり方を検討する
   - 洗い出した課題やニーズを解決するために、どのようなプロダクトが求められているかを考える
   - 既存プロダクトの拡張や、新規プロダクトの開発など、様々な選択肢を検討する

例えば、本書でサンプルとして登場します仮想のプロジェクト「LinkedPal」の場合、既存のSNSユーザーの中でも、プライバシーに配慮しつつ、友人とのつながりを大切にしたい層をターゲットとすることが考えられます。その層特有のニーズを満たすプライベートSNSというコンセプトが、LinkedPalの目的になります。

ターゲットユーザー像を具体化するために、前述のユーザーリサーチ（インタビューやアンケートなど）を活用するのが有効です。加えて、既存顧客の分析を行うことも重要でしょう。
...
これらの分析を通じて、ターゲットユーザーのペルソナを作成します。ペルソナは、プロダクト企画書にまとめ、開発の指針とします。

企画書には、以下のような項目を盛り込みます。

1. プロダクトの概要
   - プロダクトの目的や、提供する価値を簡潔に説明する
   - 既存プロダクトとの差別化ポイントを明示する

2. ターゲットユーザー
   - ペルソナを具体的に記述する
   - 各ペルソナの課題やニーズを明示する
   - 既存顧客の中のどの層をターゲットとするかを説明する
...
このように、既存顧客のニーズを起点としたプロダクト開発の場合、顧客データの分析や社内ヒアリングが重要なプロセスとなります。既存事業との関係性も考慮しつつ、ターゲット層の課題解決に最適なプロダクトのあり方を追求していくことが求められるでしょう。

プロダクト開発の第一歩として、自社の状況に合わせたアプローチでプロダクトの目的とターゲットユーザーを定義していきましょう。丁寧なユーザーリサーチと、社内での議論を重ねることで、成功に近づくはずです。
...
1. ユーザーが抱える課題の理解
   - プライバシーを守りつつ、友人とのつながりを維持したいというニーズを理解する
   - 友人ごとの情報管理という課題を特定する

2. シンプルで直感的な操作性
   - ユーザーが迷わずに目的の機能にたどり着けるようなナビゲーションを設計する
   - 複雑な機能は避け、シンプルで分かりやすいUIを心がける

3. ユーザーの行動パターンに合わせた設計
   - 友人追加やメモ機能など、ユーザーが頻繁に使う機能を中心に配置する
   - ユーザーの行動フローを考慮し、スムーズな操作を可能にする

4. プライバシー保護への配慮
   - 友人の追加方法や、公開範囲の設定など、プライバシー保護の仕組みをわかりやすく提示する
   - ユーザーがコントロール感を持てるような設計を心がける

UXデザインを適切に行うことで、ユーザーにとって価値のあるプロダクトを生み出すことができます。LinkedPalでは、ユーザーのニーズを深く理解し、それを体験設計に反映していくことが重要です。
...
1. ワイヤーフレームの作成
   - 画面のレイアウトや情報の優先順位を検討するために、ワイヤーフレームを作成する
   - ワイヤーフレームは、詳細なデザインは含まず、あくまで構成を表現するものとする
   - 主要な画面として、ホーム画面、友だちリスト画面、友だち追加画面、設定画面などを作成する

2. デザインガイドラインの策定
   - アプリ全体で一貫性のあるデザインを実現するために、デザインガイドラインを策定する
   - 色彩やタイポグラフィ、アイコンなどの使用ルールを定義する
   - Material Designなど、OSのデザインガイドラインに準拠することも検討する

3. モックアップの作成
   - ワイヤーフレームとデザインガイドラインをもとに、より詳細なデザインのモックアップを作成する
   - 実際の画面イメージに近いビジュアルデザインを表現する
   - モックアップは、エンジニアがコーディングする際の指針となる

4. インタラクションデザインの検討
   - ボタンのタップ時のフィードバックや、画面遷移のアニメーションなど、インタラクションデザインを検討する
   - ユーザーアクションに対するフィードバックを適切に設計することで、操作性を高める
   - プロトタイプツールを使って、インタラクションのシミュレーションを行うこともある

5. ユーザーテストとフィードバック
   - デザインの完成度を高めるために、ユーザーテストを実施する
   - 実際のユーザーに操作してもらい、フィードバックを収集する
   - 収集したフィードバックをもとに、デザインを改善していく

LinkedPalのUIデザインでは、ユーザーの課題解決を中心に据えながら、シンプルで直感的な操作性を実現することを目指します。デザインガイドラインを策定し、一貫性のある画面設計を行うことで、ユーザーの満足度を高めていきます。

また、デザインプロセスではエンジニアとの連携も重要です。実装の観点からのフィードバックを受けながら、デザインの実現可能性を検討していく必要があります。デザイナーとエンジニアが協力し、ユーザー体験の向上とプロダクトの完成度アップを目指していきます。

デザインとUI/UXのフェーズでは、ユーザー視点に立ち、課題解決に向けた体験設計を行うことが求められます。LinkedPalでは、プライバシー保護と友人とのつながりという、ユーザーのニーズを深く理解し、それを反映したUIデザインを実現していきます。
